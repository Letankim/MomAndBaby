...
public class BannerController extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if (path.endsWith("/MomAndBaby/staff/banner")) {
            this.getAllBanner(request, response);
        } else {
            String paths[] = path.split("/");
            int id = validate.getInt(paths[paths.length - 1]);
            if (path.startsWith("/MomAndBaby/staff/banner/delete")) {
                this.delete(request, response, id);
            } else if (path.startsWith("/MomAndBaby/staff/banner/update")) {
    ...
    private void delete(HttpServletRequest request, HttpServletResponse response, int id) {
        try {
            ...
            if (banner != null) {
                ...
                response.sendRedirect("/MomAndBaby/staff/banner?act=delete&status=" + type_message);
            } else {
    ...
    private void update(HttpServletRequest request, HttpServletResponse response) {
        try {
            ...
            String nameBanner = request.getParameter("name-banner");
            ...
            response.sendRedirect("/MomAndBaby/staff/banner?act=update&status=" + type_message);
        } catch (Exception e) {
    ...
    private void addNew(HttpServletRequest request, HttpServletResponse response) {
        try {
            ...
            response.sendRedirect("/MomAndBaby/staff/banner?act=add-new&status=" + type_message);
        } catch (Exception e) {
    ...
    private void deleteBanners(HttpServletRequest request, HttpServletResponse response) {
        try {
            ...
            if (idChecked == null) {
                response.sendRedirect("/MomAndBaby/staff/banner?act=delete&status=2");
                return;
            }
            int isDelete = 0;
            ...
            String url = "/MomAndBaby/staff/banner?act=delete&status=" + isDelete;
