...
public class VoucherController extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if (path.endsWith("/MomAndBaby/staff/voucher")) {
            this.showAllVoucher(request, response);
        } else {
            String paths[] = path.split("/");
            int id = validate.getInt(paths[paths.length - 1]);
            if (path.startsWith("/MomAndBaby/staff/voucher/update/")) {
                this.getCurrentVoucherUpdate(request, response, id);
            } else if (path.startsWith("/MomAndBaby/staff/voucher/delete/")) {
    ...
    private void deleteVoucher(HttpServletRequest request, HttpServletResponse response, int id) {
        try {
            ...
            String url = "/MomAndBaby/staff/voucher?act=delete&status=" + result;
    ...
    private void addVoucher(HttpServletRequest request, HttpServletResponse response) {
        try {
            ...
            if (this.isExist(code)) {
                response.sendRedirect("/MomAndBaby/staff/voucher?act=add-voucher&status=2");
                return;
            }
            ...
            String url = "/MomAndBaby/staff/voucher?act=add-voucher&status=" + result;
            response.sendRedirect(url);
        } catch (Exception e) {
    ...
    private void updateVoucher(HttpServletRequest request, HttpServletResponse response) {
        try {
            ...
            if (this.isExist(code) && !vCheck.getCode().equals(code)) {
                response.sendRedirect("/MomAndBaby/staff/voucher/update/" + id + "?act=update-voucher&status=2");
                return;
            }
            float value = validate.getFloat(request.getParameter("value"));
            Date start = Date.valueOf(request.getParameter("start"));
            ...
            String url = "/MomAndBaby/staff/voucher?act=update-voucher&status=" + result;
            response.sendRedirect(url);
        } catch (Exception e) {
    ...
    private void deleteVouchers(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            if (allSlugChecked == null) {
                response.sendRedirect("/MomAndBaby/staff/voucher?act=delete&status=2");
                return;
            }
            int status = 0;
            ...
            String url = "/MomAndBaby/staff/voucher?act=delete&status=" + status;
            response.sendRedirect(url);
        } catch (Exception e) {
