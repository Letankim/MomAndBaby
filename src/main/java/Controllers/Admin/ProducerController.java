...
public class ProducerController extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if (path.endsWith("/MomAndBaby/admin/producer")) {
            this.showAllProducer(request, response);
        } else {
            String paths[] = path.split("/");
            int id = validate.getInt(paths[paths.length - 1]);
            if (path.startsWith("/MomAndBaby/admin/producer/update/")) {
                this.getCurrentProducerUpdate(request, response, id);
            } else if (path.startsWith("/MomAndBaby/admin/producer/delete/")) {
    ...
    private void deleteProducer(HttpServletRequest request, HttpServletResponse response, int id) {
        try {
            ...
            String url = "/MomAndBaby/admin/producer?act=delete&status=" + result;
    ...
    private void addProducer(HttpServletRequest request, HttpServletResponse response) {
        try {
            ...
            String url = "/MomAndBaby/admin/producer?act=add-new&status=" + result;
            response.sendRedirect(url);
        } catch (Exception e) {
    ...
    private void updateProducer(HttpServletRequest request, HttpServletResponse response) {
        try {
            ...
            String url = "/MomAndBaby/admin/producer?act=update&status=" + result;
            response.sendRedirect(url);
        } catch (Exception e) {
    ...
    private void deleteProducers(HttpServletRequest request, HttpServletResponse response) {
        try {
            ...
            if (allIdChecked == null) {
                response.sendRedirect("/MomAndBaby/admin/producer?act=delete&status=0");
                return;
            }
            int status = 0;
            ...
            String url = "/MomAndBaby/admin/producer?act=delete&status=" + status;
            response.sendRedirect(url);
        } catch (Exception e) {
