...
public class BrandController extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if (path.endsWith("/MomAndBaby/admin/brand")) {
            this.showAllBrand(request, response);
        } else {
            String paths[] = path.split("/");
            int id = validate.getInt(paths[paths.length - 1]);
            if (path.startsWith("/MomAndBaby/admin/brand/update/")) {
                this.getCurrentBrandUpdate(request, response, id);
            } else if (path.startsWith("/MomAndBaby/admin/brand/delete/")) {
    ...
    private void deleteBrand(HttpServletRequest request, HttpServletResponse response, int id) {
        try {
            ...
            String url = "/MomAndBaby/admin/brand?act=delete&status=" + result;
    ...
    private void addBrand(HttpServletRequest request, HttpServletResponse response) {
        try {
            ...
            response.sendRedirect("/MomAndBaby/admin/brand?act=add-new&status=" + type_message);
        } catch (Exception e) {
    ...
    private void updateBrand(HttpServletRequest request, HttpServletResponse response) {
        try {
            ...
            String url = "/MomAndBaby/admin/brand?act=update&status=" + result;
            response.sendRedirect(url);
        } catch (Exception e) {
    ...
    private void deleteBrands(HttpServletRequest request, HttpServletResponse response) {
        try {
            ...
            if (allIdChecked == null) {
                response.sendRedirect("/MomAndBaby/admin/brand?act=delete&status=0");
                return;
            }
            int status = 0;
            ...
            String url = "/MomAndBaby/admin/brand?act=delete&status=" + status;
            response.sendRedirect(url);
        } catch (Exception e) {
